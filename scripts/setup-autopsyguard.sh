...
ENV_SCRIPT_PATH="autopsyguard-env.sh"
SKIP_SYNC=false
...
echo "This will generate a config file and an env-variable helper script."
echo "Recommended defaults are tuned for long-running monitoring."
echo
echo "Quick guidance:"
...
mkdir -p "$(dirname "$ENV_SCRIPT_PATH")"
...
{
  echo "# AutopsyGuard environment variables"
  echo "# Load with: source ./$ENV_SCRIPT_PATH"
  if [[ -n "${smtp_user// }" ]]; then
    echo "export AUTOPSYGUARD_SMTP_USER=$(shell_quote "$smtp_user")"
  fi
  if [[ -n "${smtp_password// }" ]]; then
    echo "export AUTOPSYGUARD_SMTP_PASSWORD=$(shell_quote "$smtp_password")"
  fi
  if [[ -n "${whatsapp_apikey// }" ]]; then
    echo "export AUTOPSYGUARD_WHATSAPP_APIKEY=$(shell_quote "$whatsapp_apikey")"
  fi
  if [[ -z "${smtp_user// }" && -z "${smtp_password// }" && -z "${whatsapp_apikey// }" ]]; then
    echo "# (No secret env vars were provided.)"
    echo "# Example:"
    echo "#   export AUTOPSYGUARD_SMTP_USER='your-user'"
    echo "#   export AUTOPSYGUARD_SMTP_PASSWORD='your-password'"
    echo "#   export AUTOPSYGUARD_WHATSAPP_APIKEY='your-callmebot-key'"
  fi
} > "$ENV_SCRIPT_PATH"

chmod 600 "$ENV_SCRIPT_PATH" 2>/dev/null || true

echo
echo "Setup complete."
echo "Config file: $CONFIG_PATH"
echo "Env script : $ENV_SCRIPT_PATH"
echo
echo "Next steps:"
echo "  1. source ./$ENV_SCRIPT_PATH"
echo "  2. uv run autopsyguard --config ./$CONFIG_PATH"
echo "  3. Optional debug run: uv run autopsyguard --config ./$CONFIG_PATH --verbose"
echo
