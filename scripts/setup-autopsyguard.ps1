param(
    ...
    [string]$EnvScriptPath = "autopsyguard-env.ps1",
...
Write-Host "This will generate a config file and an env-variable helper script." -ForegroundColor Cyan
Write-Host "Recommended defaults are tuned for long-running monitoring." -ForegroundColor Cyan
Write-Host ""
...
$envLines = @()
$envLines += "# AutopsyGuard environment variables"
$envLines += "# Load with: . .\$EnvScriptPath"
if (-not [string]::IsNullOrWhiteSpace($smtpUser)) {
    $envLines += '$Env:AUTOPSYGUARD_SMTP_USER = ' + (Escape-PowerShellSingleQuoted $smtpUser.Trim())
}
if (-not [string]::IsNullOrWhiteSpace($smtpPassword)) {
    $envLines += '$Env:AUTOPSYGUARD_SMTP_PASSWORD = ' + (Escape-PowerShellSingleQuoted $smtpPassword)
}
if (-not [string]::IsNullOrWhiteSpace($whatsappApiKey)) {
    $envLines += '$Env:AUTOPSYGUARD_WHATSAPP_APIKEY = ' + (Escape-PowerShellSingleQuoted $whatsappApiKey.Trim())
}
if ($envLines.Count -le 2) {
    $envLines += "# (No secret env vars were provided.)"
    $envLines += "# Example:"
    $envLines += "#   `$Env:AUTOPSYGUARD_SMTP_USER = 'your-user'"
    $envLines += "#   `$Env:AUTOPSYGUARD_SMTP_PASSWORD = 'your-password'"
    $envLines += "#   `$Env:AUTOPSYGUARD_WHATSAPP_APIKEY = 'your-callmebot-key'"
}
$envTarget = Join-Path -Path (Get-Location) -ChildPath $EnvScriptPath
...
Write-Host "Env script : $EnvScriptPath"
Write-Host ""
