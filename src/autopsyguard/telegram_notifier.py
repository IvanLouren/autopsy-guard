...
Sends plain-text alert and status messages via the CallMeBot free webhook API for Telegram.

Setup:
    1. Send a message to @CallMeBot_txtbot on Telegram
    2. Put your telegram username (e.g. @yourusername) or phone number in config as ``telegram_user``
"""
...
# CallMeBot Telegram Text API endpoint
...
class TelegramNotifier:
    ...
    # ------------------------------------------------------------------
    # Public API — mirrors EmailNotifier and WhatsAppNotifier interfaces
    # ------------------------------------------------------------------
    ...
    def _send_message(self, text: str) -> bool:
        ...
        def _do_send() -> bool:
            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    logger.debug(
                        "Telegram send attempt %d/%d", attempt, max_attempts
                    )
                    ...
                    with urllib.request.urlopen(req, timeout=15) as resp:
                        ...
                        if status == 200:
                            ...
                            logger.warning(
                                "Telegram API returned status %d (attempt %d)",
                                status, attempt,
                            )
            ...
            logger.error(
                "❌ Telegram message failed after %d attempts: %s",
